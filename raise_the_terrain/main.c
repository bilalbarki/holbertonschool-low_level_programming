...
int gameloop(SDL_Window* window, SDL_Renderer* gRenderer, TTF_Font *font, SDL_Texture *texture)
{
    ...
    else
    {
        ...
        int hyp, x_center, y_center, j, i, m, k, color_fill = 0;
        ...
        if (font == NULL)
        {
            printf( "Error loading font: %sn", TTF_GetError());
            return 1;
        }
        quit = 1;
        x_offset = 0;
...
{
    /*cartX -= centerX;
    cartX = cartX * cos(rotate*0.1) - cartY * sin(rotate*0.1);
    cartX += centerX;*/
